(function () {
    ...
    function _dsToHex(rgb) {
        ...
        const alphaMatch = rgb.match(/rgba\(\s*(\d+),\s*(\d+),\s*(\d+),\s*([\d.]+)\s*\)/);
        if (alphaMatch) {
            const a = parseFloat(alphaMatch[4]);
            if (a < 0.05) return null; // fully transparent — discard
            if (a < 0.999) {
                // Semi-transparent — preserve as rgba string
                return `rgba(${alphaMatch[1]}, ${alphaMatch[2]}, ${alphaMatch[3]}, ${Math.round(a * 100) / 100})`;
            }
            // Fully opaque rgba — convert to hex
            return '#' + [alphaMatch[1], alphaMatch[2], alphaMatch[3]].map(n => parseInt(n).toString(16).padStart(2, '0').toUpperCase()).join('');
        }
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        ...
        if (request.action === 'START_DS_BUILDER') {
            // Prefer stepKey lookup so display order doesn't affect extraction
            const stepDef = request.stepKey
                ? DS_BUILDER_STEP_DEFINITIONS.find(s => s.stepKey === request.stepKey)
